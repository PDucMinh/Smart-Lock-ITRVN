...
void system_manager_init(driver_button_t *db, driver_led_t *dl)
{
    ...
    // Init Button
    db->button_type = DRIVER_BUTTON_PULL_UP;
    db->button_io = (DRIVER_MCU_PORT_A << 4) | (DRIVER_MCU_PIN_0);    // PA0
    driver_button_init(db);

    // Init Led
    dl->led_type = DRIVER_LED_KCOMMON;
    dl->led_io = (DRIVER_MCU_PORT_D << 4) | (DRIVER_MCU_PIN_12);  // PD12
    driver_led_init(dl);
    (dl + 1)->led_type = DRIVER_LED_KCOMMON;
    (dl + 1)->led_io = (DRIVER_MCU_PORT_D << 4) | (DRIVER_MCU_PIN_13);  // PD13
    driver_led_init(dl + 1);
    (dl + 2)->led_type = DRIVER_LED_KCOMMON;
    (dl + 2)->led_io = (DRIVER_MCU_PORT_D << 4) | (DRIVER_MCU_PIN_14);  // PD14
...
void system_manager_loop(driver_button_t *db, driver_led_t *dl)
{
    ...
    static system_state_t currentState = SYSTEM_IDLE;
